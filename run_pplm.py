...
def null_shape_vector(old):
    ...
    for j in range(len(old)):
        ...
        adapted.append(tuple(temp))
    
    return tuple(adapted)



def adapt_shape(past):
...
def reverse_adapt_shape(past_adapted,bkp):
    
    past=[]
    for k in range(len(past_adapted)):

        temp=[0,0]
        temp[0]=past_adapted[k][0]
        temp[1]=past_adapted[k][1]
        past.append(tuple(temp))
        
  

        past.append(tuple(bkp[2*k+1]))
...
def perturb_past(
    ...
    for k in past:
        temp_copy=[]
        for j in k:
            temp_copy.append(torch.clone(j).detach())
        past_bkp.append(tuple(temp_copy))
    
    past_bkp=tuple(past_bkp)
        
            

    #past_bkp=(past)
    
    #past_bkp=torch.clone(past).detach()
    
    
    

    
    past=adapt_shape(past)
    ...
    #grad_accumulator=null_shape_vector(past)
    ...
    for i in range(num_iterations):
        ...
        perturbed_past_reversed=reverse_adapt_shape(perturbed_past,past_bkp)
                
        
        
     
        lm_output = model(last, past_key_values=perturbed_past_reversed) ### balise ###
    ...
    pert_past_reversed=reverse_adapt_shape(pert_past,past_bkp)


   
    return pert_past_reversed, new_accumulated_hidden, grad_norms, loss_per_iter
...
def get_bag_of_words_indices(bag_of_words_ids_or_paths: List[str], tokenizer) -> List[List[List[int]]]:
    ...
    for id_or_path in bag_of_words_ids_or_paths:
        ...
        bow_indices.append([tokenizer.encode(word.strip()) for word in words])
    return bow_indices
...
def run_pplm_example(
    ...
    model = GPTNeoForCausalLM.from_pretrained(pretrained_model,output_hidden_states=True)
    model.to(device)
    model.eval()
